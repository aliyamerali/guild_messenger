...
RSpec.describe 'Get User\'s Messages Endpoint' do
  describe 'happy path' do
    ...
    it 'returns recipient\'s last 100 messages from a given sender' do
      limit = "100"
      body =  {"sender_id": @sender_a_id, "recipient_id": @recipient_id, "limit": limit}

      get '/api/v1/messages', params: body, as: :json
      ...
      expect(output[:data].first[:sender_id]).to eq(sender_a_id)
      expect(output[:data].first[:created_at]).to eq(DateTime.new(2021,8,15))
      expect(output[:data].last[:sender_id]).to eq(sender_a_id)
      expect(output[:data].last[:created_at]).to eq(DateTime.new(2021,7,01))
    end
